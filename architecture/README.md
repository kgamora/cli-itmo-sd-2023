...
5. Разбиение строк на лексемы с помощью `Lexer`. Если ввод не пустой и `Lexer` успешно обработал строки, то результат передается в `Parser` , иначе пользователю сообщается об ошибке и то выполняется переход к пункту 1.
...
Класс `Substitution` имеет метод `substitute`, принимающий строку и выполняющий подстановку следующие действия:
...
2. Если вторая лексема **соответствует** символу `=`, то создается соответствующий наследник класса `Executable`. Остальные лексемы передаются в качестве аргументов.
