...
5. Конкатинация строк, после подстановки. Затем разбиение входной строки с помощью `Lexer`. Если `Lexer` успешно обработал строку то переходим к следующему пункту, иначе выводим ошибку и переходим к пункту 1.

6. Построение `Executable` с помощью `Parser`.

7. Последовательное исполнение `Executable` с передачей вывода предыдущих команд последующим при наличии пайпов.

8. В случае успешного завершения всех `Executable` печатается вывод. В случае возникновения ошибки во время исполнения одного из `Executable` выполнение прекращается и печается информации об ошибке.
...
В данном разделе описаны представители класса `Executable`, поставлемые по умолчанию.
...
Класс `GlobalExecutable` отвечает за вызов команд, которые не реализованы в данном приложении по умолчанию.
